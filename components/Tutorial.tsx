...
export const Tutorial: React.FC = () => {
  ...
  useEffect(() => {
    const t = setTimeout(() => setVisible(false), 4000); // 4 seconds total (1s fade-in + 2s visible + 1s fade-out)
    return () => clearTimeout(t);
  }, []);

  return (
    <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 ui-panel px-8 py-4 flex items-center gap-8 pointer-events-none transition-opacity duration-1000 ${visible ? 'opacity-100' : 'opacity-0'}`}>
